    ...
    <DashboardLayout>
        <div class="p-6 md:p-12 min-h-full bg-slate-50/50">
            ...
            <div class="md:hidden mb-6">
                <h2 class="text-xl font-black text-slate-900 tracking-tight uppercase">{{ currentPage.title }}</h2>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
                ...
                <div class="xl:col-span-4 space-y-6">
                    <div v-if="hasData" class="space-y-4">
                        ...
                        <div v-if="contactInfo.phone" class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Telefone</p>
                                <h3 class="text-sm font-black text-slate-800">{{ contactInfo.phone }}</h3>
                            </div>
                        </div>
                        ...
                        <div v-if="contactInfo.email" class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">E-mail</p>
                                <h3 class="text-sm font-black text-slate-800 lowercase">{{ contactInfo.email }}</h3>
                            </div>
                        </div>
                        ...
                        <div v-if="contactInfo.address" class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-[9px] font-black text-slate-400 uppercase tracking-widest">Localização</p>
                                <h3 class="text-[11px] font-bold text-slate-700 uppercase leading-snug">{{ contactInfo.address }}</h3>
                            </div>
                        </div>
                        ...
                        <div v-if="contactInfo.hours.br || contactInfo.hours.pt" class="bg-white p-6 rounded-3xl shadow-sm border border-slate-200">
                            ...
                            <div class="space-y-4">
                                <div v-if="contactInfo.hours.br" class="flex items-center justify-between border-b border-slate-50 pb-3">
                                    <span class="flex items-center gap-2 text-[10px] font-black text-slate-500 uppercase tracking-tight">
                                        <CountryFlag name="Brasil" class-name="w-5 h-3" /> BRASIL
                                    </span>
                                    <span class="text-xs font-bold text-slate-800 uppercase">{{ contactInfo.hours.br }}</span>
                                </div>
                                <div v-if="contactInfo.hours.pt" class="flex items-center justify-between">
                                    ...
                                    <span class="text-xs font-bold text-slate-800 uppercase">{{ contactInfo.hours.pt }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div v-if="contactInfo.whatsapp || contactInfo.socials.length > 0" class="bg-blue-600 p-8 rounded-[2.5rem] text-white shadow-xl relative overflow-hidden group">
                        ...
                        <h4 class="text-[10px] font-black uppercase tracking-[0.2em] text-blue-100 mb-6">Fale Conosco</h4>
                        ...
                        <a v-if="contactInfo.whatsapp" :href="contactInfo.whatsapp" target="_blank" class="flex items-center gap-4 bg-white/10 hover:bg-white/20 p-4 rounded-2xl transition-all mb-6 border border-white/10">
                            ...
                            <span class="text-xs font-black uppercase tracking-widest">Suporte WhatsApp</span>
                        </a>
                    ...
                    <div v-if="!hasData" class="bg-slate-100/50 border-2 border-dashed border-slate-200 rounded-[2.5rem] p-8 text-center">
                        <p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4">Aguardando Configuração</p>
                        <Link v-if="authUser.is_master" :href="route('admin.settings.index')" class="text-xs font-bold text-blue-600 hover:underline">Configurar dados corporativos</Link>
                    </div>
                </div>
                ...
                <div class="xl:col-span-8 bg-white rounded-[2.5rem] shadow-sm border border-slate-200 p-8 md:p-12 relative overflow-hidden">
                    ...
                    <div class="relative z-10">
                        <h2 class="text-2xl font-black text-slate-900 uppercase tracking-tight mb-2">Envie uma Mensagem</h2>
                        ...
                        <form @submit.prevent="submitContact" class="space-y-6">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 opacity-60">
                                <div class="space-y-2">
                                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Nome (Automático)</label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                            <User class="w-4 h-4" />
                                        </div>
                                        <input :value="authUser.name" readonly class="w-full pl-11 bg-slate-50 border-slate-200 rounded-2xl text-xs font-bold text-slate-500 h-12" />
                                    </div>
                                </div>
                                <div class="space-y-2">
                                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">E-mail (Automático)</label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                            <Mail class="w-4 h-4" />
                                        </div>
                                        <input :value="authUser.email" readonly class="w-full pl-11 bg-slate-50 border-slate-200 rounded-2xl text-xs font-bold text-slate-500 h-12" />
                                    </div>
                                </div>
                                <div v-if="authUser.company_name" class="space-y-2 lg:col-span-2">
                                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Empresa</label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                            <Building class="w-4 h-4" />
                                        </div>
                                        <input :value="authUser.company_name" readonly class="w-full pl-11 bg-slate-50 border-slate-200 rounded-2xl text-xs font-bold text-slate-500 h-12" />
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="space-y-6">
                                <div class="space-y-2">
                                    <label class="block text-[10px] font-black text-slate-600 uppercase tracking-widest pl-1">Assunto do Contato</label>
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                ...
                                <div class="space-y-2">
                                    <label class="block text-[10px] font-black text-slate-600 uppercase tracking-widest pl-1">Sua Mensagem</label>
                            ...
                            <div class="pt-6 flex justify-end">
                                <button type="submit" :disabled="form.processing" class="bg-[#0f172a] hover:bg-slate-800 text-white font-black text-xs uppercase tracking-[0.2em] py-5 px-12 rounded-2xl shadow-xl shadow-slate-200 transition-all active:scale-95 disabled:opacity-50 flex items-center gap-3">
            ...
            <div class="mt-20 text-center">
                <p class="text-[10px] font-black text-slate-300 uppercase tracking-[0.3em]">Gerenciador Analítico JRSpice • 2024</p>
            </div>
        </div>
    </DashboardLayout>
</template>
