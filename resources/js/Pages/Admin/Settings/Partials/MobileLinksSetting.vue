...
import { Smartphone, Save, Loader2, CheckCircle2, Layout } from 'lucide-vue-next';
...
const form = useForm({
    settings: {
        mobile_user_pages: Array.isArray(props.initialMobilePages) ? props.initialMobilePages : []
    }
});
...
const togglePage = (pageName) => {
    const index = form.settings.mobile_user_pages.indexOf(pageName);
    if (index > -1) {
        form.settings.mobile_user_pages.splice(index, 1);
    } else {
        form.settings.mobile_user_pages.push(pageName);
    }
};
</script>
        ...
        <form @submit.prevent="submit" class="p-6 space-y-6">
            ...
            <div v-else-if="availablePages.length > 0" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                <div
                    ...
                    :class="form.settings.mobile_user_pages.includes(page.name)
                ...
                >
                    <div class="flex items-center space-x-3">
                        <div
                            ...
                            :class="form.settings.mobile_user_pages.includes(page.name) ? 'bg-indigo-500 text-white' : 'bg-white border border-slate-200 text-slate-400'"
                        >
                            <Layout class="w-5 h-5" />
                        </div>
                        <div>
                            <span class="text-xs font-bold uppercase tracking-wider block" :class="form.settings.mobile_user_pages.includes(page.name) ? 'text-indigo-700' : 'text-slate-500'">
                    ...
                    <div class="flex items-center">
                        <div
                            ...
                            :class="form.settings.mobile_user_pages.includes(page.name) ? 'bg-indigo-500 border-indigo-500 scale-110' : 'border-slate-300 bg-white'"
                        >
                            <CheckCircle2 v-if="form.settings.mobile_user_pages.includes(page.name)" class="w-3 h-3 text-white" />
                        </div>
                    </div>
                </div>
            </div>
