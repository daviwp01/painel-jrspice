                    ...
                    >
                        ...
                        <div class="flex items-center justify-between px-4 py-4 border-b border-slate-50">
                            <div class="flex items-center min-w-0 flex-1">
                                <div
                                    class="h-10 w-10 rounded-lg border flex-shrink-0 flex items-center justify-center font-bold text-sm shadow-sm capitalize"
                                ...
                                <div class="ml-3 min-w-0 flex-1">
                                    <div class="flex items-center space-x-1.5">
                                        <span class="text-sm font-bold text-slate-800 tracking-tight truncate">{{ user.name }}</span>
                            ...
                            <div class="flex-shrink-0 ml-3">
                                <div class="inline-flex items-center px-2.5 py-1 rounded-lg text-[9px] font-bold uppercase tracking-wider shadow-sm"
                        ...
                        <div class="px-4 py-3 space-y-2.5">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center text-slate-600 bg-slate-50/50 px-2.5 py-1 rounded-lg border border-slate-100">
                                    ...
                                    <span class="text-[11px] font-bold uppercase tracking-tighter">{{ user.last_login }}</span>
                                </div>
                            </div>
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center text-slate-800 bg-slate-50/50 px-2.5 py-1 rounded-lg border border-slate-100">
                                    ...
                                    <span class="text-[11px] font-bold uppercase tracking-tighter">{{ user.last_activity }}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-between px-4 py-3 bg-slate-50/50 border-t border-slate-100">
                            <div class="flex flex-col items-start">
                                ...
                                <div v-if="user.notified_at" class="inline-flex items-center space-x-1.5 bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-lg border border-indigo-100">
                                    <Mail class="w-3 h-3" />
                                    <span class="text-[10px] font-bold uppercase tracking-wider">{{ user.notified_at }}</span>
                                </div>
                            ...
                            <div class="flex flex-col items-end">
                                ...
                                <div v-if="user.clicked_at" class="inline-flex items-center space-x-1.5 bg-emerald-50 text-emerald-600 px-2.5 py-1 rounded-lg border border-emerald-100">
                                    ...
                                    <span class="text-[10px] font-bold uppercase tracking-wider">{{ user.clicked_at }}</span>
                                </div>
                                <div v-else class="text-[10px] font-bold text-slate-300 italic">---</div>
                            </div>
                        </div>
                        ...
                        <table class="min-w-full divide-y divide-slate-100">
                            <thead>
                                <tr class="bg-slate-50/50">
                                    <th scope="col" class="px-8 py-6 text-left text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">{{ $t('User') }}</th>
                                    <th scope="col" class="px-8 py-6 text-center text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">{{ $t('Status') }}</th>
                                    <th scope="col" class="px-8 py-6 text-left text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">{{ $t('Last Login') }}</th>
                                    <th scope="col" class="px-8 py-6 text-left text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">{{ $t('Last Activity') }}</th>
                                    <th scope="col" class="px-8 py-6 text-center text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">{{ $t('Email Notification') }}</th>
                                    <th scope="col" class="px-8 py-6 text-center text-xs font-bold text-slate-400 uppercase tracking-[0.2em]">{{ $t('Email Interaction') }}</th>
                                </tr>
                            </thead>
                                ...
                                <tr v-for="user in users.data" :key="user.id" class="hover:bg-indigo-50/20 group transition-all duration-300">
                                    <td class="px-8 py-5 whitespace-nowrap">
                                        <div class="flex items-center">
                                            <div
                                                class="h-12 w-12 rounded-lg border flex items-center justify-center font-bold text-sm shadow-sm transition-transform duration-300 group-hover:scale-110 capitalize"
                                                :class="getAvatarColor(user.name)"
                                            >
                                                {{ user.name.charAt(0) }}
                                            </div>
                                            <div class="ml-4">
                                                <div class="flex items-center space-x-1.5">
                                                    <span class="text-sm font-bold text-slate-800 tracking-tight group-hover:text-indigo-600 transition-colors">{{ user.name }}</span>
                                    ...
                                    <td class="px-8 py-5 whitespace-nowrap">
                                        <div class="flex items-center text-slate-800 bg-slate-50/50 px-3 py-2 rounded-xl border border-slate-100 w-fit group-hover:bg-indigo-50 group-hover:border-indigo-100 transition-colors">
                                            ...
                                            <span class="text-xs font-bold uppercase tracking-tighter">{{ user.last_activity }}</span>
                                        </div>
                                    </td>
                                    <td class="px-8 py-5 whitespace-nowrap text-center">
